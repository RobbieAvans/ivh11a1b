...
public class HallReservationSeviceImpl implements HallReservationService {
    ...
    /**
    @Override
    public HallReservation update(HallReservationAPIWrapper newReservation, HallReservation oldReservation) {
        List<HallOption> oldHallOptions = oldReservation.getHallOptions();
        List<HallOption> newHallOptions = newReservation.getHallOptions();
//        
//        for (HallOption newHallOption : newHallOptions) {
//            if (oldHallOptions.contains(newHallOption)) {
//                // Remove from both arrays
//                oldHallOptions.remove(newHallOption);
//                newHallOptions.remove(newHallOption);
//            }
//        }
//                
//        // Remove oldHallOptions that are left
//        for (HallOption oldHallOption : oldHallOptions) {
//            
//        }
//        
//        // Decorate with newHallOptions
        
        if (oldHallOptions.size() > 1) {
            HallReservationOption oldReservationComplete = (HallReservationOption) oldReservation;
            
              
        }
        
        // Get the hall where we will save it on
        Hall hall = newReservation.getHall();
